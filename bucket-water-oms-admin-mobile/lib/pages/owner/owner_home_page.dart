...
class _OwnerHomePageState extends State<OwnerHomePage> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      print('[OwnerHomePage] 正在加载水站首页数据...');

      final dashboardService = DashboardService();
      final dashboardData = await dashboardService.getDashboardData();
      print('[OwnerHomePage] Dashboard数据加载完成');

      final inventoryService = InventoryService();
      final products = await inventoryService.getProducts();
      print('[OwnerHomePage] 商品列表加载完成，共 ${products.length} 个商品');

      final stationService = StationService();
      final stationInfo = await stationService.getStationInfo();
      print('[OwnerHomePage] 水站信息加载完成: ${stationInfo.name}');

      final unreadCount = await _notificationService.getUnreadCount();
      print('[OwnerHomePage] 未读通知数: $unreadCount');
      ...
      if (mounted) {
        ...
        print('[OwnerHomePage] 数据加载完成');
      }
    } on ApiException catch (e) {
      print('[OwnerHomePage] API异常: ${e.message}');
      if (mounted) {
        setState(() {
  ...
  Widget _buildHeader(AppState appState) {
    final stationName = _stationInfo?.name ?? appState.currentStationName;
        ...
        children: [
          ...
          Expanded(
            child: Column(
              ...
              children: [
                Text(
                  stationName.isNotEmpty ? stationName : '水站',
  ...
  Widget _buildFinancialCard() {
    final balance = _stationInfo?.balance?.toStringAsFixed(2) ??
        _dashboardData?.accountBalanceText ??
        '0.00';
    final creditUsed = _stationInfo?.usedCredit?.toStringAsFixed(0) ??
        _dashboardData?.usedCredit?.toString() ??
        '0';
    final creditLimit = _stationInfo?.creditLimit?.toStringAsFixed(0) ??
        _dashboardData?.creditLimit?.toString() ??
        '0';
    final paymentType = _stationInfo?.paymentTypeText ?? '月结';
    final paymentDays = _stationInfo?.paymentDays ?? 30;
  ...
  Widget _buildEmptyBucketWarning() {
    final emptyBarrels =
        _stationInfo?.oweBuckets ?? _dashboardData?.emptyBarrels ?? 0;
    ...
    return Column(
      ...
      children: [
        ...
        if (_productList.isEmpty)
          ...
          ..._productList.take(3).map((product) => Padding(
  ...
  Widget _buildProductCard(ProductModel product) {
    ...
    return GestureDetector(
      onTap: () => Navigator.push(
          context, MaterialPageRoute(builder: (_) => const ProductListPage())),
      child: AppCard(
        padding: const EdgeInsets.all(16),
        child: Row(
          children: [
