...
class _OwnerOrdersPageState extends State<OwnerOrdersPage> {
  ...
  void _navigateToOrderDetail(OrderModel order) {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (_) => OwnerOrderDetailPage(orderId: order.id ?? ''),
      ),
    ).then((_) => _loadOrders());
  }
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            ElevatedButton(onPressed: _loadOrders, child: const Text('重新加载')),
          ],
        ),
      );
    }
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            Text('暂无订单数据', style: AppTextStyles.body1.copyWith(color: AppColors.textSecondary)),
          ],
        ),
      );
    }
  ...
  Widget _buildOrderCard(OrderModel order) {
    ...
    if (order.items != null && order.items!.isNotEmpty) {
      ...
      productText = '18.9L × ${order.totalQuantity ?? 0}桶';
    }

    return AppCard(
      padding: const EdgeInsets.all(20),
      borderColor: order.status == 'dispatched' || order.status == 'delivering'
