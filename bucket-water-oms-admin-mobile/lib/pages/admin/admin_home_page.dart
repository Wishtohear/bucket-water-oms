...
class _AdminHomePageState extends State<AdminHomePage> {
    ...
    return ListView(
      ...
      children: [
        ...
        _buildSidebarItem(
          icon: Icons.category,
          label: '销售政策',
          onTap: () => _navigateToPage(const AdminPoliciesPage()),
        ),
        _buildSidebarItem(
          icon: Icons.inventory,
          label: '库存管理',
          onTap: () => _navigateToPage(const AdminInventoryPage()),
        ),
        _buildSidebarItem(
          icon: Icons.account_balance_wallet,
          label: '财务管理',
          onTap: () => _navigateToPage(const AdminFinancePage()),
        ),
        _buildSidebarItem(
          icon: Icons.bar_chart,
          label: '报表统计',
          onTap: () => _navigateToPage(const AdminReportsPage()),
        ),
        const Divider(height: 32),
        _buildSidebarItem(
          icon: Icons.settings,
    ...
    return AppBar(
      ...
      actions: [
        IconButton(
          ...
          onPressed: () => _navigateToPage(const AdminReportsPage()),
          color: AppColors.textSecondary,
        ),
  ...
  Widget _buildChartToggleButton(String label, bool isActive) {
    return GestureDetector(
      onTap: () {
        setState(() {
          _selectedPeriod = label == '按周' ? 'week' : 'month';
        });
        _loadData();
      },
      child: Container(
          ...
          builder: (context, constraints) {
            if (constraints.maxWidth > 600) {
                  ...
                  return Expanded(
                    child: Padding(
                      padding: EdgeInsets.only(
                        ...
                        left: item == quickActions.first ? 0 : 0,
                      ),
              ...
              return Wrap(
                ...
                children: quickActions.map((item) {
                  return SizedBox(
                    width: (constraints.maxWidth - 8) / 2 - 4,
                    child: _buildQuickActionCard(item),
                  );
                }).toList(),
              );
