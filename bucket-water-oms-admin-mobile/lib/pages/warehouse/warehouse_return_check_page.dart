...
class WarehouseReturnCheckPage extends StatefulWidget {
  ...
  const WarehouseReturnCheckPage({super.key, this.returnId});
...
class _WarehouseReturnCheckPageState extends State<WarehouseReturnCheckPage> {
  ...
  Future<void> _loadReturnData() async {
    ...
    try {
      ...
      if (warehouseId != null) {
        final detail =
            await _returnService.getReturnDetail(widget.returnId!, warehouseId);
        if (detail != null && mounted) {
          setState(() {
            _returnData = {
              'returnId': detail.returnNo ?? detail.id ?? '',
              'driverName': detail.driverName ?? '未知司机',
              'driverCode': detail.driverCode ?? '',
              'orderId': detail.orderNo ?? detail.orderId ?? '',
              'deliveryTime': _formatTime(detail.deliveryTime),
              'reportBucketCount': detail.reportedBuckets ?? 0,
              'oweBucketCount': detail.owedBuckets ?? 0,
              'replenishRequest': detail.replenishmentRequest ?? '暂无',
              'replenishStock': '充足',
        ...
        children: [
          Row(
            children: [
              ...
              GestureDetector(
                onTap: () {
                  final phone = _returnData['driverPhone'] as String?;
