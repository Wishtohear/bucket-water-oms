...
import '../../services/warehouse_audit_logs_service.dart';

class WarehouseOperationLogsPage extends StatefulWidget {
  const WarehouseOperationLogsPage({super.key});

  @override
  State<WarehouseOperationLogsPage> createState() =>
      _WarehouseOperationLogsPageState();
}

class _WarehouseOperationLogsPageState extends State<WarehouseOperationLogsPage> {
  final _searchController = TextEditingController();
  final WarehouseAuditLogsService _auditService = WarehouseAuditLogsService();

  int _currentPage = 1;
  int _totalPages = 1;
  bool _isLoading = false;
  String? _errorMessage;
  List<OperationLogModel> _logs = [];
  int _totalLogs = 0;
  ...
  Future<void> _loadLogs({bool resetPage = true}) async {
    ...
    try {
      final response = await _auditService.getOperationLogs(
        username: _searchController.text.isNotEmpty
            ? _searchController.text
            : null,
        action:
            _selectedAction != '全部' ? _getActionValue(_selectedAction) : null,
  ...
  Widget _buildLogItem(OperationLogModel log) {
  ...
  void _showLogDetail(OperationLogModel log) {
            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  _buildDetailItem('操作类型', log.actionText),
                  _buildDetailItem('模块', log.moduleText),
